...
//      for(int j=0; j<7; ++j) { averageElectronRatePerRoll += eleBkg[j]*yy_helper; yy_helper *= yy_glob; }
      ...
      averageElectronRatePerRoll = eleBkg[0] * rSqrtR*  TMath::Exp(eleBkg[1]/rSqrtR) + eleBkg[2]/rSqrtR + eleBkg[3]/(sqrt(yy_glob));
...
//      for(int j=0; j<7; ++j) { averageNeutralRatePerRoll += neuBkg[j]*yy_helper; yy_helper *= yy_glob; }
      ...
      averageNeutralRatePerRoll = neuBkg[0] * yy_glob* TMath::Exp(neuBkg[1]/rSqrtR) + neuBkg[2]/rSqrtR + neuBkg[3]/(sqrt(yy_glob));
